...
run_chain <- function(shapefile_path, 
  ...
  # Clean up any previous global settings
  cleanup_globals()
  ...
  # Set global weight variables for scoring functions
  assign("WEIGHT_CUT_EDGES", weight_cut_edges, envir = .GlobalEnv)
  assign("WEIGHT_COUNTY_SPLITS", weight_county_splits, envir = .GlobalEnv)
  assign("WEIGHT_MEAN_MEDIAN", weight_mean_median, envir = .GlobalEnv)
  assign("WEIGHT_EFFICIENCY_GAP", weight_efficiency_gap, envir = .GlobalEnv)
  assign("WEIGHT_DEM_SEATS", weight_dem_seats, envir = .GlobalEnv)
  assign("WEIGHT_COMPETITIVENESS", weight_competitiveness, envir = .GlobalEnv)
  
  assign("EXPONENT_CUT_EDGES", exponent_cut_edges, envir = .GlobalEnv)
  assign("EXPONENT_COUNTY_SPLITS", exponent_county_splits, envir = .GlobalEnv)
  assign("EXPONENT_MEAN_MEDIAN", exponent_mean_median, envir = .GlobalEnv)
  assign("EXPONENT_EFFICIENCY_GAP", exponent_efficiency_gap, envir = .GlobalEnv)
  assign("EXPONENT_DEM_SEATS", exponent_dem_seats, envir = .GlobalEnv)
  assign("EXPONENT_COMPETITIVENESS", exponent_competitiveness, envir = .GlobalEnv)
  
  assign("TARGET_MEAN_MEDIAN", target_mean_median, envir = .GlobalEnv)
  assign("TARGET_EFFICIENCY_GAP", target_efficiency_gap, envir = .GlobalEnv)
  assign("TARGET_DEM_SEATS", target_dem_seats, envir = .GlobalEnv)
  
  assign("USE_ROBUST_EFFICIENCY_GAP", use_robust_efficiency_gap, envir = .GlobalEnv)
  assign("ELECTION_WIN_PROB_AT_55", election_win_prob_at_55, envir = .GlobalEnv)
  assign("COMPETITIVENESS_MAXIMIZE", competitiveness_maximize, envir = .GlobalEnv)
  
  # Initialize bunking lists if needed
  if (!is.null(bunking_lists)) {
    ...
    assign("BUNKING_LISTS", bunking_lists, envir = .GlobalEnv)
  }
  ...
  validate_tracking(tracking, shp, counties, bunking_lists)
  ...
  result <- calculate_map_metrics(graph, assignment, node_pops, shp, counties, ideal_pop, bunking_lists, timers)
  current_metrics <- result$metrics
  timers <- result$timers
  timers <- add_time(timers, "score_calculation", t0)
  
  current_score <- calculate_map_score(current_metrics)
  ...
  for (step in 1:num_steps) {
    ...
    t0 <- start_timer()
    result <- calculate_map_metrics(graph, proposed_assignment, node_pops, shp, counties, ideal_pop, bunking_lists, timers)
    proposed_metrics <- result$metrics
    timers <- result$timers
    timers <- add_time(timers, "score_calculation", t0)
    
    proposed_score <- calculate_map_score(proposed_metrics)
    ...
    if (should_print) {
      if (!verbose_console) {
        ...
        # Determine label based on robust mode
        eg_label <- if (!is.null(current_metrics$is_robust_eg) && current_metrics$is_robust_eg) {
          "EG ADJ"
        } else {
          "EF GAP"
        }
  ...
  if (use_optimization) {
    ...
    cat(sprintf("Score: %.1f to %.1f (D=%.1f)\n", 
